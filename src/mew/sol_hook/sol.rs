...
use {
    ...
    crate::mew::{
        sol_hook::compute_budget::{ix_set_compute_unit_limit, ix_set_compute_unit_price},
        swqos::{
            blox::{Bloxroute, SubmitOpts, SubmitProtection},
            helius::HeliusSender,
            jito::JitoClient,
            nextblock::NextBlock,
            temporal::TemporalSender,
            zero_slot::ZeroSlot,
        },
    },
    crate::{log, warn},
    ...
    solana_program::{
        ...
        nonce::{State, state::Versions as NonceVersions},
        pubkey::Pubkey,
        system_instruction,
    },
...
impl SolHook {
    pub fn new(rpc_client: String) -> Self {
        Self {
            rpc_client: Arc::new(RpcClient::new(rpc_client)),
        }
    }
    ...
    pub async fn get_token_info(&self, mint: &Pubkey) -> anyhow::Result<TokenInfo> {
        ...
        let creator = md
            .creators
            .as_ref()
            .and_then(|v| v.first().map(|c| c.address));
    ...
    pub async fn get_token_program_id(&self, token_address: &Pubkey) -> anyhow::Result<Pubkey> {
        let info = self
            .rpc_client
            .get_account_with_commitment(token_address, CommitmentConfig::processed())
            .await?;
        Ok(info
            .value
    ...
    pub async fn send_with_jito(
        ...
        let jito_tip_ix =
            solana_program::system_instruction::transfer(&payer.pubkey(), &rta, 1_000_000);
    ...
    pub async fn send_with_jito_bundle(
        ...
        ixs.insert(
            2,
            solana_program::system_instruction::transfer(&payer.pubkey(), &tip_acc, tip_lamports),
        );
    ...
    pub async fn spray_with_nextblock(
        ...
        let tip_acc = clients[0].get_tip_account();
        let tip_ix =
            solana_program::system_instruction::transfer(&payer.pubkey(), &tip_acc, 1_000_000);
        ixs.insert(2, tip_ix);
    ...
    pub async fn spray_with_helius(
        ...
        let tip_acc = clients[0].get_tip_account();
        let tip_ix =
            solana_program::system_instruction::transfer(&payer.pubkey(), &tip_acc, 500_000);
        ixs.insert(2, tip_ix);
        let blockhash = self
            .rpc_client
            .get_latest_blockhash_with_commitment(CommitmentConfig::processed())
    ...
    pub async fn spray_with_zero_slot(
        ...
        let tip_acc = clients[0].get_tip_account();
        let tip_ix =
            solana_program::system_instruction::transfer(&payer.pubkey(), &tip_acc, 3_200_000);
        ixs.insert(0, tip_ix);
    ...
    pub async fn spray_with_all(
        ...
        ixs_hl.push(solana_program::system_instruction::transfer(
            &payer.pubkey(),
            &hl_tip_acc,
            tip_lamports,
        ));
        ...
        ixs_jito.push(solana_program::system_instruction::transfer(
            &payer.pubkey(),
            &jito_rta,
            tip_lamports,
        ));
        ...
        ixs_nb.push(solana_program::system_instruction::transfer(
            &payer.pubkey(),
            &nb_tip_acc,
            tip_lamports,
        ));
        ...
        ixs_tmp.push(solana_program::system_instruction::transfer(
            &payer.pubkey(),
            &tmp_tip_acc,
            tip_lamports,
        )); // 0.001 SOL
        ...
        ixs_bx.push(solana_program::system_instruction::transfer(
            &payer.pubkey(),
            &bx_tip_acc,
            tip_lamports,
        ));
...
mod tests {
    ...
    async fn test_exists() {
        ...
        let exists = sol
            .exists(&Pubkey::try_from("FL4CKfetEWBMXFs15ZAz4peyGbCuzVaoszRKcoVt3WfC").unwrap())
            .await
            .unwrap_or(false);
    ...
    async fn test_get_balance() {
        ...
        let sol = SolHook::new(rpc);
        let balance = sol
            .get_balance(&Pubkey::try_from("FL4CKfetEWBMXFs15ZAz4peyGbCuzVaoszRKcoVt3WfC").unwrap())
            .await;
    ...
    async fn test_get_token_balance() {
        ...
        let token_balance = match sol
            .get_token_balance(
                &Pubkey::try_from("Esj1iA4Gbrt5rRCQVv5x7wAtr1NXZ4smWBp7FYmHasa7").unwrap(),
                &USDC_MINT,
            )
            .await
        {
    ...
    async fn test_get_token_metadata() {
        let rpc = "https://api.mainnet-beta.solana.com".to_string();
        let sol = SolHook::new(rpc);
        let (metadata, pda) = sol
            .get_token_metadata(
                &Pubkey::from_str("FKugW5fK2g3b2rm79TiL7GTKMJKE25xG9LYbUPnpump").unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_get_token_info() {
        ...
        let token_info = sol
            .get_token_info(
                &Pubkey::from_str("6entYcGxY1n2XkpZtTM62nreoS4rPByJSkoKHQjwruMg").unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_pump_fun() {
        ...
        while let Some(msg) = rx.recv().await {
            ...
            // println!("Message: {:?}", msg);
            let lines: std::slice::Iter<'_, String> = msg.logs.iter();
    ...
    async fn test_grpc_pump() {
        ...
        let mut grpc_client = SolHook::grpc_connect(&grpc_url, Some(&grpc_token))
            .await
            .unwrap();
        let pump_address = Pubkey::from_str("6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P").unwrap();
        let accounts = vec![pump_address];
        let mut stream = SolHook::grpc_subscribe_transactions(&mut grpc_client, &accounts)
            .await
            .unwrap();
    ...
    async fn test_pump_fetch_state() {
        ...
        let bonding_curve =
            Pubkey::from_str("usAdAyXTZHo8U5JH4ykNi2Cr6fbWyFYXkkhKqFeSXA6").unwrap();
    ...
    async fn test_pump_swap_buy() {
        ...
        let (ixs, recent_fees) = pump
            ...
            .unwrap();
        let sig = sol
            .send(ixs, &keypair, recent_fees, Some(250_000))
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_swap_sell() {
        ...
        let pump = PumpSwap::new(Arc::new(keypair.insecure_clone()), Arc::new(sol.clone()));
        let (state, price) = pump.fetch_price(&pool).await.unwrap();
        let (ixs, _recent_fees) = pump
            .sell(&mint, &pool, &state.coin_creator, 100, 1.3, price)
            .await
            .unwrap();
        let sig = sol
            .send(ixs, &keypair, 5_000_0, Some(250_000))
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_sell() {
        ...
        let (mut ixs, recent_fees) = pump
            .sell(&mint, &bonding_curve, &state.creator, 100, 1.5, price)
            .await
            .unwrap();
    ...
    async fn test_pump_buy_jito() {
        ...
        let sig = sol
            .send_with_jito(&jito_client, ixs, &keypair, recent_fees, 70_000)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_jito_bundle() {
        ...
        let sig = sol
            .spray_with_jito_bundle(ixs, &keypair, recent_fees, 70_000, &jito_clients, 1_000_000)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_nextblock() {
        ...
        let sig = sol
            .spray_with_nextblock(ixs, &keypair, recent_fees, 70_000, &clients)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_helius() {
        ...
        let sig = sol
            .spray_with_helius(ixs, &keypair, recent_fees, 70_000, &clients)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_zero_slot() {
        let config = config();
        let clients =
            &ZeroSlot::multiple_new(&ZERO_SLOT_ENDPOINTS, url_decode("").unwrap().to_string());
        ...
        let sig = sol
            .spray_with_zero_slot(ixs, &keypair, recent_fees, 100_000, &clients)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_temporal() {
        let config = config();
        let clients = &TemporalSender::multiple_new(
            &TEMPORAL_HTTP_ENDPOINTS,
            url_decode("").unwrap().to_string(),
        );
        let mint = Pubkey::from_str("AhMLyXcAzkpHbfH2AxMEdiUCzSmsdtbWSvh953Y9pump").unwrap();
        let bonding_curve =
            Pubkey::from_str("usAdAyXTZHo8U5JH4ykNi2Cr6fbWyFYXkkhKqFeSXA6").unwrap();
        ...
        let sig = sol
            .spray_with_temporal(ixs, &keypair, recent_fees, 70_000, clients, 1_000_000)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_blox() {
        let config = config();
        let clients =
            &Bloxroute::multiple_new(&BLOX_ENDPOINTS, url_decode("").unwrap().to_string());
        let mint = Pubkey::from_str("AhMLyXcAzkpHbfH2AxMEdiUCzSmsdtbWSvh953Y9pump").unwrap();
        let bonding_curve =
            Pubkey::from_str("usAdAyXTZHo8U5JH4ykNi2Cr6fbWyFYXkkhKqFeSXA6").unwrap();
        let sol = SolHook::new(config.rpc_url.clone());
        ...
        let sig = sol
            .spray_with_blox(ixs, &keypair, recent_fees, 90_000, clients, 1_000_000)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_pump_buy_all() {
        ...
        let zero_slot_clients =
            &ZeroSlot::multiple_new(&ZERO_SLOT_ENDPOINTS, url_decode("").unwrap().to_string());
        zero_slot_clients[0].warmup().await;
        let temporal_clients = &TemporalSender::multiple_new(
            &TEMPORAL_HTTP_ENDPOINTS,
            url_decode("").unwrap().to_string(),
        );
        ...
        let sig = sol
            .spray_with_all(
                ...
                1_100_000,
                None,
                Some(Pubkey::from_str(&nonce_account).unwrap()),
            )
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    ...
    async fn test_tx_single() {
        ...
        let sig = sol
            .spray_with_jito(ixs, &keypair, recent_fees, 25_000, &jito_clients)
            .await
            .unwrap();
        println!("Sig: https://solscan.io/tx/{}", sig);
    }
    ...
    async fn test_fees() {
        ...
        let sol = SolHook::new(rpc.clone());
        let addresses = vec![];
        sol.fetch_priority_fee(&PriorityFeeLevel::Low, &addresses)
            .await
            .unwrap();
    }
    ...
    async fn test_compare_rpc_grpc() {
        ...
        let mut grpc_reqs = SolHook::grpc_connect(&grpc_url, Some(&grpc_token))
            .await
            .unwrap();
        let mut grpc_subs = SolHook::grpc_connect(&grpc_url, Some(&grpc_token))
            .await
            .unwrap();
        let mut stream = SolHook::grpc_subscribe_slot_channel(&mut grpc_subs)
            .await
            .unwrap();
        ...
        while let Some(msg) = stream.next().await {
            let slot: yellowstone_grpc_proto::prelude::subscribe_update::UpdateOneof =
                msg.unwrap().update_oneof.unwrap();
            match slot {
    ...
    async fn test_buy_with_vac() {
        const FAV_PLACES: &[&str] = &["Frankfurt"];
        let config = config();
        ...
        let mut stream = SolHook::grpc_subscribe_slot_channel(&mut grpc_client)
            .await
            .unwrap();
        let mut irv = vec![];
        let mut last_slot = 0;
            ...
            match slot {
                UpdateOneof::Slot(slot_update) => {
                    ...
                    let quad = leaders
                        .iter()
                        .filter(|(s, _, _)| {
                            *s == slot || *s == slot + 1 || *s == slot + 2 || *s == slot + 3
                        }) // slot, city, country
                        .collect::<Vec<_>>();
    ...
    async fn test_nonce_account() {
        ...
        let sig = sol
            .create_nonce_account(&keypair, &nonce_account, &keypair.pubkey())
            .await
            .unwrap();
