...
impl MewSnipe {
    ...
    pub async fn refresh_creators_loop(&self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            if algo_creators.is_empty() && last_creators == 0 {
                consecutive_errors += 1;
                warn!(
                    "refresh_creators_loop: initial creator load returned 0 creators (attempt {consecutive_errors}), retrying before accepting empty set"
                );
                let backoff = std::cmp::min(60 * consecutive_errors as u64, 300);
                tokio::time::sleep(std::time::Duration::from_secs(backoff)).await;
                continue;
